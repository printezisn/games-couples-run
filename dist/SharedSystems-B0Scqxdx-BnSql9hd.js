import { D as l, P as O, ac as ee, a7 as p, E as C, r as We, a2 as te, a1 as re, b, ad as Le, a as F, a9 as B, l as ae, u as ne, ae as se, G as m, af as Ve, ag as $e, j as v, C as je, K as Ne, g as ie, p as j, h as qe, ah as Ke, U as w, S as oe, ai as Ye, x as de, aj as Xe, X as N, ak as Je, _ as q, a0 as k, aa as U, al as Qe, ab as M, k as Ze, am as S, an as ue, ao as le, ap as ce, aq as K, ar as et } from "./index-D9OaVkLm.js";
import { w as he, k as Y, U as tt, v as rt } from "./colorToUniform-CoBYoR1j-vhgs0e86.js";
...
class it extends at {
  constructor(e) {
    const { sprite: t, ...r } = e, n = new et(t.texture), s = new ie({
...
const E = class fe {
  ...
  buildEnd(e) {
    ...
    for (const r in t) {
      const n = t[r], s = n.geometry;
      s.indexBuffer.setDataWithSize(n.indexBuffer, n.indexSize, !0), s.buffers[0].setDataWithSize(n.attributeBuffer.float32View, n.attributeSize, !1);
    }
  }
  upload(e) {
...
function ot(a, e) {
  const t = a.root, r = a.instructionSet;
  r.reset();
  const n = e.renderPipes ? e : e.batch.renderer, s = n.renderPipes;
  s.batch.buildStart(r), s.blendMode.buildStart(), s.colorMask.buildStart(), t.sortableChildren && t.sortChildren(), ve(t, r, n, !0), s.batch.buildEnd(r), s.blendMode.buildEnd(r);
}
function G(a, e, t) {
  const r = t.renderPipes ? t : t.batch.renderer;
  a.globalDisplayStatus < 7 || !a.includeInBuild || (a.sortableChildren && a.sortChildren(), a.isSimple ? dt(a, e, r) : ve(a, e, r, !1));
}
function dt(a, e, t) {
  if (a.renderPipeId) {
    const r = a, { renderPipes: n, renderableGC: s } = t;
    n.blendMode.setBlendMode(r, a.groupBlendMode, e), n[r.renderPipeId].addRenderable(r, e), s.addRenderable(r, e), r.didViewUpdate = !1;
  }
  if (!a.renderGroup) {
    const r = a.children, n = r.length;
    for (let s = 0; s < n; s++)
      G(r[s], e, t);
  }
}
function ve(a, e, t, r) {
  const { renderPipes: n, renderableGC: s } = t;
  if (!r && a.renderGroup)
    n.renderGroup.addRenderGroup(a.renderGroup, e);
  else {
    for (let u = 0; u < a.effects.length; u++) {
      const c = a.effects[u];
      n[c.pipe].push(c, a, e);
    }
    const i = a, o = i.renderPipeId;
    o && (n.blendMode.setBlendMode(i, i.groupBlendMode, e), n[o].addRenderable(i, e), s.addRenderable(i, e), i.didViewUpdate = !1);
    const d = a.children;
    if (d.length)
      for (let u = 0; u < d.length; u++)
        G(d[u], e, t);
    for (let u = a.effects.length - 1; u >= 0; u--) {
      const c = a.effects[u];
      n[c.pipe].pop(c, a, e);
    }
  }
...
class _e {
  ...
  execute(e) {
    ...
    let s = (t = this._maskStackHash)[n] ?? (t[n] = 0);
    e.action === "pushMaskBegin" ? (r.renderTarget.ensureDepthStencil(), r.stencil.setStencilMode(p.RENDERING_MASK_ADD, s), s++, r.colorMask.setMask(0)) : e.action === "pushMaskEnd" ? (e.inverse ? r.stencil.setStencilMode(p.INVERSE_MASK_ACTIVE, s) : r.stencil.setStencilMode(p.MASK_ACTIVE, s), r.colorMask.setMask(15)) : e.action === "popMaskBegin" ? (r.colorMask.setMask(0), s !== 0 ? r.stencil.setStencilMode(p.RENDERING_MASK_REMOVE, s) : (r.renderTarget.clear(null, B.STENCIL), r.stencil.setStencilMode(p.DISABLED, s)), s--) : e.action === "popMaskEnd" && (e.inverse ? r.stencil.setStencilMode(p.INVERSE_MASK_ACTIVE, s) : r.stencil.setStencilMode(p.MASK_ACTIVE, s), r.colorMask.setMask(15)), this._maskStackHash[n] = s;
  }
  destroy() {
...
function Et(a, e) {
  for (const t in a.attributes) {
    const r = a.attributes[t], n = e[t];
    n ? (r.format ?? (r.format = n.format), r.offset ?? (r.offset = n.offset), r.instance ?? (r.instance = n.instance)) : ne(`Attribute ${t} is not present in the shader, but is present in the geometry. Unable to infer attribute details.`);
  }
  ct(a);
}
function ct(a) {
  const { buffers: e, attributes: t } = a, r = {}, n = {};
  for (const s in e) {
    const i = e[s];
    r[i.uid] = 0, n[i.uid] = 0;
  }
...
class Ht {
  ...
  _initUniformGroup(e) {
    ...
    if (!r) {
      const n = Object.keys(e.uniformStructures).map((o) => e.uniformStructures[o]), s = this._adaptor.createUboElements(n), i = this._generateUboSync(s.uboElements);
...
function pt(a, e, t, r, n, s) {
  const i = s ? 1 : -1;
  return a.identity(), a.a = 1 / r * 2, a.d = i * (1 / n * 2), a.tx = -1 - e * a.a, a.ty = -i - t * a.d, a;
}
...
function Ce(a, e = []) {
  e.push(a);
  for (let t = 0; t < a.renderGroupChildren.length; t++)
    Ce(a.renderGroupChildren[t], e);
  return e;
}
function mt(a, e, t) {
  const r = a >> 16 & 255, n = a >> 8 & 255, s = a & 255, i = e >> 16 & 255, o = e >> 8 & 255, d = e & 255, u = r + (i - r) * t, c = n + (o - n) * t, h = s + (d - s) * t;
  return (u << 16) + (c << 8) + h;
}
const P = 16777215;
function we(a, e) {
  return a === P || e === P ? a + e - P : mt(a, e, 0.5);
}
const vt = new C(), J = ce | ue | le;
function Ge(a, e = !1) {
  gt(a);
  const t = a.childrenToUpdate, r = a.updateTick++;
  for (const n in t) {
    const s = Number(n), i = t[n], o = i.list, d = i.index;
    for (let u = 0; u < d; u++) {
      const c = o[u];
      c.parentRenderGroup === a && c.relativeRenderGroupDepth === s && Pe(c, r, 0);
    }
    H(o, d), i.index = 0;
  }
  if (e)
    for (let n = 0; n < a.renderGroupChildren.length; n++)
      Ge(a.renderGroupChildren[n], e);
}
function gt(a) {
  const e = a.root;
  let t;
...
function Pe(a, e, t) {
  ...
  const n = a.parent;
  if (n && !n.renderGroup ? (t = t | a._updateFlags, a.relativeGroupTransform.appendFrom(
    r,
    n.relativeGroupTransform
  ), t & J && Q(a, n, t)) : (t = a._updateFlags, a.relativeGroupTransform.copyFrom(r), t & J && Q(a, vt, t)), !a.renderGroup) {
    const s = a.children, i = s.length;
    for (let u = 0; u < i; u++)
      Pe(s[u], e, t);
...
function Q(a, e, t) {
  ...
  t & le && (a.groupBlendMode = a.localBlendMode === "inherit" ? e.groupBlendMode : a.localBlendMode), t & ce && (a.globalDisplayStatus = a.localDisplayStatus & e.globalDisplayStatus), a._updateFlags = 0;
}
function xt(a, e) {
  ...
  return a.structureDidChange = n, n;
}
const _t = new v();
class Re {
  constructor(e) {
  ...
  render({ container: e, transform: t }) {
    ...
    }), Me(e.renderGroup, d), d.uniformBatch && d.uniformBatch.renderEnd(), t && e.renderGroup.localTransform.copyFrom(o), e.parent = r, e.renderGroup.renderGroupParent = n;
  }
  destroy() {
    this._renderer = null;
  }
...
function bt(a) {
  const { list: e, index: t } = a.childrenRenderablesToUpdate;
  for (let r = 0; r < t; r++) {
    const n = e[r];
    n.didViewUpdate && a.updateRenderable(n);
  }
  H(e, t);
}
class Be {
...
const R = {
  ...
  async base64(e) {
    ...
    if (n.toDataURL !== void 0)
      return n.toDataURL(R[t], r);
    if (n.convertToBlob !== void 0) {
      const s = await n.convertToBlob({ type: R[t], quality: r });
  ...
  canvas(e) {
    ...
    const n = r.textureGenerator.generateTexture(e), s = r.texture.generateCanvas(n);
    return n.destroy(), s;
  }
  /**
  ...
  pixels(e) {
    e = this._normalizeOptions(e);
    const t = e.target, r = this._renderer, n = t instanceof m ? t : r.textureGenerator.generateTexture(e), s = r.texture.getPixels(n);
    return t instanceof C && n.destroy(), s;
  }
  /**
...
class Oe {
  ...
  generateTexture(e) {
    ...
    const r = e.resolution || this._renderer.resolution, n = e.antialias || this._renderer.view.antialias, s = e.target;
    let i = e.clearColor;
    i ? i = Array.isArray(i) && i.length === 4 ? i : ae.shared.setValue(i).toArray() : i = St;
...
function wt(a) {
  if (!Z) {
    if (oe.get().getNavigator().userAgent.toLowerCase().indexOf("chrome") > -1) {
      const e = [
        `%c  %c  %c  %c  %c PixiJS %c v${K} (${a}) http://www.pixijs.com/
...
`,
    ...
    } else globalThis.console && globalThis.console.log(`PixiJS ${K} - ${a} - http://www.pixijs.com/`);
    Z = !0;
  }
}
class W {
...
const L = class He {
  ...
  run() {
    var e;
    const t = performance.now(), r = this._managedRenderables, n = this._renderer.renderPipes;
    let s = 0;
    for (let i = 0; i < r.length; i++) {
      ...
      o._lastInstructionTick !== u && t - o._lastUsed > this.maxUnusedTime ? (o.destroyed || n[o.renderPipeId].destroyRenderable(o), o._lastInstructionTick = -1, s++, o.off("destroyed", this._removeRenderable, this)) : r[i - s] = o;
    }
    r.length = r.length - s;
  }
  destroy() {
...
const V = class ze {
  ...
  init(e) {
    e = { ...ze.defaultOptions, ...e }, this.checkCountMax = e.textureGCCheckCountMax, this.maxIdle = e.textureGCAMaxIdle ?? e.textureGCMaxIdle, this.active = e.textureGCActive;
  }
  /**
